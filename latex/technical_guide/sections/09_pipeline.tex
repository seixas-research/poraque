...
Standard VASP does not write \file{EXTCAR}. Depending on it would make the
pipeline unusable for anyone without the patched build, so the default is to
\textbf{compute} the potential from \file{POSCAR}, \file{INCAR} and
\file{POTCAR} using the tabulated pseudopotential of
Chapter~\ref{ch:vasp}. Setting \opt{use\_vasp\_extcar: true} reads a reference
file instead, and errors clearly if none is present.

The default costs essentially nothing in fidelity. Measured across five gold
structures:
...
\section{Universal training}
\label{sec:universal}

\textbf{One model per task is trained on the combined data of every
structure.} This is the deployable artefact.
...
The run writes a single \file{models/ext2chg.pt} and \file{models/chg2tau.pt}.

\begin{pwarn}
With \opt{holdout: null} every structure is in training, so the reported
metrics are \textbf{training fit}. They show the model can represent the data;
they are not a generalisation estimate. Name structures in \opt{holdout}, or
use the cross-validation of Section~\ref{sec:kfold}, for that.

Measured on five gold structures: universal training fit gives relative $L^2$
of $0.0057$ (\opt{ext2chg}) and $0.0133$ (\opt{chg2tau}), against
cross-validated values roughly five times larger. That gap is the memorisation
margin, and quoting the training-fit number as a result would be a serious
overstatement.
\end{pwarn}
...
Path & Contents \\
\midrule
\file{models/*.pt} & trained operators (one per task in universal mode) \\
