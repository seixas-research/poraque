...
Poraqu\^e computes the external ionic potential natively. Reference \file{EXTCAR} and \file{TAUCAR} files were used during development to
...
themselves are ordinary VASP~6.6.1 output in every other respect.
...
\begin{itemize}
  \item \textbf{Hard truncation.} For $G \ge$ \opt{PSGMA2} the \emph{whole}
    ...
    the table. For the platinum potential \opt{PSGMAX} $=75.6\ \text{\AA}^{-1}$
...
Confirmed against the platinum \file{POTCAR}: the value is $75.5890395431569$, the
...
for platinum feeds the \opt{PSCENC} energy correction and does not enter the
...
\end{equation}
Doing so for platinum shows $f(G)$ decaying far more slowly than a Gaussian at low
...
set is pure platinum. And \opt{ISPIN=2} writes a second block in both \file{CHGCAR}
...
On the Ag--Pt--Pt space this returns $11$, $11$ and $10$ with a residual of
...
Ag--Pt--Pt densities, the two constructions differ from \emph{each other} by
\begin{equation}
