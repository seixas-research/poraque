...
The spectral tensor is the model. Counted on the shipped
\file{platinum\_W16-M8-L3} itself --- its \opt{ext2chg} half, $C=16$, $M=8$,
$L=3$, $C_P=64$, and $C_{\mathrm{in}}=1$, $C_{\mathrm{out}}=2$ because the
platinum data is spin-polarised and the density it predicts is
$(\rho, m)$ --- with a complex weight counted as two parameters, which is what
\pyclass{FNO3d.n\_parameters} does and therefore what the training log prints:
...
Read-out        & Eqs.~\eqref{eq:idx-read1}--\eqref{eq:idx-read2} & $1\,218$ & $0.04\%$ \\
...
normalisation together are $0.15\%$, and the read-out --- the whole of
\opt{projection\_channels} --- is $0.04\%$.

The second output channel is almost free, and the table says why: $C_{\rm out}$
appears only in Eq.~\eqref{eq:idx-read2}, so predicting $m$ alongside $\rho$
costs $C_P + 1 = 65$ parameters, or $0.002\%$. Nothing in the spectral tensor
knows how many fields it is carrying.

\noindent
Each row is its equation's index ranges multiplied out. Note the leading
$2$ on the spectral row: $R$ is a \emph{complex} tensor, and a complex weight
is two real parameters.
\begin{align}
  \text{spectral}  &: \; \mathbf{2} \times L \times 4 \times C^2 \times m_1 m_2 m_3 \\
...
\begin{pnote}
\textbf{Why the training log prints roughly twice what the spectral formula
gives.} For the shipped model ($C=16$, $M=8$, $L=3$),
\begin{equation*}
  4 \times C^2 \times M^3 \times L
  = 4 \times 256 \times 512 \times 3 = 1\,572\,864,
\end{equation*}
and that is right --- it is the number of \emph{complex} weights in $R$,
exactly what the three ``spectral tensors'' hold. Two things stand between it
and the $3\,152\,418$ the log reports:

\begin{enumerate}
  \item \textbf{a factor of two}, because
    \pyclass{FNO3d.n\_parameters} counts a complex weight as the two real
    numbers it is: $1\,572\,864 \to 3\,145\,728$;
  \item \textbf{everything that is not $R$}: $6\,690$ parameters --- the FiLM
    projections ($3\times1\,056$), the cell encoder ($1\,312$), the read-out
    ($1\,218$), the pointwise convolutions ($3\times272$), GroupNorm
    ($3\times32$) and the lift ($80$).
\end{enumerate}

$3\,145\,728 + 6\,690 = 3\,152\,418$, which is the table above. A
one-channel model would report $65$ fewer: its last convolution is
$C_P C_{\mathrm{out}} + C_{\mathrm{out}} = 65$ rather than $130$.
\end{pnote}
