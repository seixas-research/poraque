\chapter{Fine-tuning}
\label{ch:finetuning}

A model fitted across a broad dataset is a good \emph{starting point} for a
specific material family, and a poor substitute for one. Fine-tuning continues
that fit on the smaller set at a much lower learning rate: what the base model
learned about the general map is kept, and only the specialisation is learned.

\begin{lstlisting}
# 1. the base model, trained across everything you have
poraque-train --config configs/train.yaml

# 2. specialise it on one family
poraque-train --config configs/train.yaml \
    --fine-tune --pretrained models/poraque_models.poraque \
    --root data/oxides --checkpoint-dir models/oxides
\end{lstlisting}

\section{What comes from the checkpoint}

Two things are taken from the base model rather than from this run's
configuration, and both matter.

\textbf{The architecture}, inferred from the stored tensors. A \opt{width} or
\opt{modes} left over in the config that disagreed with the weights could only
load mismatched tensors, so the tensors are the authority --- the \opt{model}
section is ignored for the \emph{shape} of a fine-tuned network.

\textbf{The normalizations.} The datasets are re-pointed at the checkpoint's
transforms, discarding the ones just fitted to the new data.

\begin{pwarn}
Refitting the normalizations would rescale the network's inputs out from under
weights trained against the old scale, and the model would spend the fine-tune
relearning the scale instead of the chemistry. It also means the new data must
fall in a comparable range: fine-tuning on a family whose densities are an
order of magnitude away is transfer learning in name only.
\end{pwarn}

\section{Freezing the lifting path}

The lifting map embeds the input field into the network's channel space before
any operator acts on it. It is the most general part of the model, so it is the
part least in need of specialising --- and freezing it removes parameters a
small dataset could otherwise overfit. The cell encoder is frozen with it: that
embeds the lattice, a property of the input rather than of the mapping.

The \textbf{projection head stays trainable}. It decodes to physical units,
which is precisely what differs between material families.

\begin{lstlisting}
fine_tuning:
  freeze_lifting_layers: true
\end{lstlisting}

The run reports what was actually frozen, so it never has to be inferred:

\begin{lstlisting}
      fine-tuning from : models/poraque_models.poraque
      architecture     : inferred from the checkpoint (4,202,001 parameters)
      transforms       : taken from the checkpoint
      frozen           : lifting path, 1,392 parameters;
                         4,200,609 remain trainable
      learning rate    : 1e-05 (fine-tuning; base training uses 0.002)
\end{lstlisting}

\begin{ptip}
Frozen parameters are kept out of the optimiser entirely, not merely denied a
gradient. AdamW's decoupled weight decay applies without one, so a frozen
weight left in the optimiser would shrink towards zero every step --- quietly
undoing the pre-training the freeze was meant to preserve.
\end{ptip}

\section{Configuration}

\begin{center}
\begin{tabular}{@{}p{3.9cm}p{3.7cm}p{7.4cm}@{}}
\toprule
Key & Default & Meaning \\
\midrule
\opt{enable} & \opt{false} & Start from \opt{pretrained\_checkpoint} instead of
a fresh initialisation. \\
\opt{pretrained\_} \opt{checkpoint} & \file{models/poraque\_} \file{models.poraque}
& Base bundle to adapt. \\
\opt{learning\_rate} & \opt{1e-05} & Replaces \opt{training.learning\_rate} for
this run. \\
\opt{freeze\_lifting\_} \opt{layers} & \opt{false} & Hold the input lifting
path fixed. Ignored under \opt{use\_lora}. \\
\opt{use\_lora} & \opt{false} & Freeze everything and learn a low-rank
correction; see below. \\
\opt{lora\_rank} & \opt{8} & $r$; cost and capacity are both linear in it. \\
\opt{lora\_alpha} & \opt{16.0} & The update is scaled by
\opt{lora\_alpha / lora\_rank}. \\
\opt{lora\_dropout} & \opt{0.0} & Dropout on the adapter's input. \\
\bottomrule
\end{tabular}
\end{center}

\section{LoRA --- adapting a model that does not fit}

\opt{use\_lora: true} freezes every trained weight and learns a rank-$r$
correction beside the model's \textbf{dense ($1\times1\times1$) lifting and
projection} convolutions:

\begin{equation*}
  W' = W + \frac{\alpha}{r}\, B A ,
  \qquad A \in \mathbb{R}^{r \times d_{\mathrm{in}}},
  \quad B \in \mathbb{R}^{d_{\mathrm{out}} \times r} .
\end{equation*}

On the shipped model that is \textbf{1\,320 trainable parameters against
3\,152\,353 frozen --- 0.042\,\%}, so the optimiser state and the saved file
shrink by three orders of magnitude and the fine-tune fits where a full one
does not:

\begin{lstlisting}
      LoRA             : 3 adapted layer(s), rank 8, alpha 16
      trainable        : 1,320 of 3,153,673 parameters (0.042%); the rest is frozen
      NOTE: the checkpoint will hold the ADAPTER only and name this base;
            it cannot be loaded without poraque_models.poraque.
\end{lstlisting}

$B$ starts at \textbf{zero}, so $BA = 0$ and the adapted model \emph{is} the
base model until the first optimiser step --- a fine-tune that began anywhere
else would already have discarded some of what it set out to adapt.

\begin{pwarn}
\textbf{The spectral weights are not adapted.} They hold ${\sim}99.8\,\%$ of
the parameters and look like the obvious target, but a rank-$r$ factorisation
of a 5-index complex kernel is a \emph{choice of which axes to pair} rather
than one decomposition, and adapting them would also stop being cheap.

What LoRA says here is precise: \emph{keep the learned operator, re-fit how
fields enter and leave it.} A family whose operator genuinely differs from the
base model's --- not merely its input and output scales --- is out of its
reach, and the honest answer there is a full fine-tune. It buys memory, not
generality.
\end{pwarn}

\begin{pnote}
\textbf{A LoRA checkpoint is not self-contained.} It holds the adapter and
records where its base lives, which is the whole economy of the method --- the
frozen tensors are already on disk in the model being adapted. Move or delete
that base and the fine-tune cannot be loaded; the error says so and names the
file. \opt{freeze\_lifting\_layers} is ignored under LoRA, which freezes
everything anyway, and the run says so rather than appearing to apply both.
\end{pnote}

The fine-tuning learning rate is much smaller by design: the base rate would
walk the weights away from the solution being adapted before a small dataset
could constrain them.

\section{Output, and the \opt{.poraque} format}
\label{sec:poraque}

Poraqu\^e writes its models as \opt{.poraque}, after the project rather than
after the architecture inside it:

\begin{center}
\begin{tabular}{@{}lp{8.4cm}@{}}
\toprule
File & Contents \\
\midrule
\file{models/poraque\_models.poraque} & The universal model: both operators in one
file. \\
\file{models/poraque\_finetuned.poraque} & A fine-tune, specialised to one
family. \\
\bottomrule
\end{tabular}
\end{center}

The container is a \opt{torch.save} dictionary of exactly three entries:

\begin{lstlisting}[language=Python]
checkpoint = {
    "model_state_dict": {"ext2chg": ..., "chg2tau": ...},
    "config": {...},                   # the resolved training config
    "paw_profiles": {78: {...}, ...},  # PAW data per atomic number
}
\end{lstlisting}

\opt{model\_state\_dict} holds, per task, everything that rebuilds the
operator: the weights, the architecture record, both normalisations and the
$\delta$-density baseline --- weights alone would decode to a field in the
wrong units. \opt{config} is the resolved configuration as a plain dictionary.
\opt{paw\_profiles} holds, per element and keyed by $Z$, the radial core charge
density read from the POTCAR that built $\vext$ (\opt{r} in \AA,
\opt{core\_density} and \opt{pseudo\_core\_density} in e/\AA$^3$,
\opt{core\_electrons}) and the \opt{augmentation} occupancies that
\opt{--add-paw} writes.

The POTCAR stores the core as $r^{2}\rho_{00}(r)$ on a mesh in \AA, and states
neither. Both were pinned by measurement: $\sqrt{4\pi}\int$ of the table is
exactly $Z - Z_{\mathrm{val}}$ for every dataset checked --- 68.0000 for Pt ---
and the pseudized core rejoins the all-electron one at \opt{RPACOR} converted
to \AA.

\begin{pwarn}
The core density is derived from the POTCAR, which is licensed with VASP. A
checkpoint carrying it carries pseudopotential data; mind that before sharing
one outside a group that holds the licence.
\end{pwarn}

The extension is a label, not a different serialisation. Nothing inspects it
when loading, so a checkpoint under any name loads fine.

A fine-tune is \textbf{never} written over the base model. The two coexist in
the same directory, and a run that would genuinely overwrite its own base is
refused --- before training starts, not after.

The PDF report leads its Configuration section with the fine-tuning facts and
carries a caveat at the top, so a reader cannot mistake a specialised model's
scores for a general one's. The checkpoint's \opt{config} records
\opt{fine\_tuning.pretrained\_checkpoint}, the learning rate and whether the
lifting layers were frozen, so a checkpoint can always be traced back to its
parent.

\begin{pnote}
The extension has been changed twice: \opt{.pth} first became \opt{.pfno}, and
\opt{.pfno} became \opt{.poraque} on 2 September 2026. Neither change touched
the container. The container itself changed on 16 September 2026, from a
\opt{poraque-bundle-1} payload with one top-level entry per task and a metadata
block to the three keys above, and \textbf{a file in the old layout is not
read}: it raises, names the layout, and asks for a retrain. If the
\opt{.poraque} file is
absent and a \opt{.pfno}, \opt{.pth} or \opt{.pt} of the same stem is present,
that one is used and the substitution is announced --- an existing trained model
should not become invisible because a default filename changed. The search runs
the other way too, which matters for a LoRA checkpoint: it stores the path of
the base it adapts, and that path may name a file since renamed. Rename the file
to silence the notice.
\end{pnote}
