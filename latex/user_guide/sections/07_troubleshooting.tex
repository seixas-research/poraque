...
\begin{description}
  ...
  \item[``Unknown key(s) in section \ldots''] A configuration key was
    removed. \opt{mode}, \opt{holdout}, \opt{use\_vasp\_extcar},
    \opt{train\_fraction} and \opt{split} no longer exist; delete them, or
    start again from \file{configs/train.yaml}. The split is now one number,
    \opt{valid\_fraction}, and \file{EXTCAR} is always computed.
  \item[``No valence charge for \ldots''] No \file{POTCAR} was found. Supply one,
    or pass \opt{--zval Pt=11}.
...
\begin{description}
  ...
  \item[The numbers seem too good] Check whether anything was held out. With
    \opt{valid\_fraction: 0} they are a training fit. The report's
