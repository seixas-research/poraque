...
Eleven configurations ship with the source --- three at the top of
...
\file{train\_ext2paw.yaml} & The \opt{ext2paw} task on the same data: the
pseudo-density and every atom's PAW occupancies. \\
...
the ones at the top working, because those were the ones anyone loaded.
\end{pnote}
...
Key & Default & Meaning \\
\midrule
\opt{task} & \opt{all} & Which map to train: \opt{ext2chg}, \opt{chg2tau},
\opt{all} for both in sequence, or \opt{ext2paw} (\S\ref{sec:paw}). \\
\bottomrule
\end{tabular}
\end{center}
...
epochs --- see below. \\
\opt{spin} & \opt{auto} & Train on the magnetisation as a second channel
...
Value & What happens \\
\midrule
\opt{auto} & Ask the sources whether their \file{CHGCAR}s carry a second grid
block, and settle the channel count for the whole dataset before the cache is
written. \\
\opt{true} & Demand one: a dataset with no magnetisation \textbf{raises}. \\
\opt{false} & Drop it deliberately, and say so in the log. \\
\bottomrule
\end{tabular}
\end{center}

This is the switch for spin-polarised training, and there is no other. A
\file{CHGCAR} from an \mbox{\opt{ISPIN = 2}} run stores two grids, the total density
$\rho$ and the magnetisation $m = \rho_\uparrow - \rho_\downarrow$; with
\opt{spin} resolved on, both reach the cache, \opt{ext2chg} predicts them as
channels 0 and 1 of one output, and \opt{chg2tau} reads both. How many channels
there are is a property of the data, not of the architecture, which is why the
key lives in \opt{data} and why \opt{model} has no second one: two switches
could disagree, and one of them would then be quietly ignored. An unpolarised
member of a spin set carries $m \equiv 0$, so a mixed archive trains one
two-channel operator rather than two datasets. The cache tag says which it
holds (\file{res48\_potcar\_spin}).
...
\opt{charge} is the data term with the prediction substituted into the density
channel alone, the magnetisation left at its reference; \opt{mag} is the same
the other way round. Both keep the objective's norm and \textbf{its
denominator}, the norm of the whole $(\rho, m)$ target. That choice is forced
rather than convenient: a relative error of $m$ on its own divides by $m$,
which is identically zero on every non-magnetic cell, so it would be undefined
on precisely the materials most datasets are made of. With a shared denominator
the parts of an $L^2$ objective add in quadrature,
$\mathrm{data} = \sqrt{\mathrm{charge}^2 + \mathrm{mag}^2}$ per sample, and the
...
\opt{train loss} remains the total the optimiser stepped on, physics terms
included.

\begin{pnote}
Both parts are in the metrics JSON's \opt{history}, as
\opt{train\_loss\_charge} and \opt{train\_loss\_magnetisation}, and exist only
for a two-channel \opt{ext2chg} target --- a one-channel run prints neither
column. The magnetisation gets its own parity figure,
\file{<task>\_parity\_magnetisation}, on linear axes because $m$ changes sign,
and its own held-out number, \opt{magnetisation\_relative\_l2}; the report's
...
\opt{paw} & \opt{\{enable: false\}} & The \opt{ext2paw} operator ---
one block, see below. \\
...
\opt{ext2paw} maps $\vext$ to two things at once: the pseudo-density on the
grid, as \opt{ext2chg} does, and every atom's \textbf{augmentation occupancies}
$\rho(\ell\ell'LM)$ --- the one-centre PAW terms VASP writes after the grid,
which with the pseudo-density are what an all-electron density is rebuilt from.
They are not a field. There are 138 per atom for \opt{PAW\_PBE Pt}, whose
projector channels are d, d, s, s, p, p in the POTCAR's order, and one set per
density channel, so a spin-polarised dataset keeps the magnetisation's set too.
...
Key & Default & Meaning \\
\midrule
\opt{enable} & \opt{false} & Train \opt{ext2paw}: required by
\opt{task.type: ext2paw}, and adds it to an \opt{all} run. \\
\opt{width} & \opt{64} & The FNO backbone's channels. \\
\opt{modes} & \opt{16} & Its retained Fourier modes per axis. \\
\opt{n\_layers} & \opt{4} & Its Fourier layers. \\
\opt{readout\_g\_max} & \opt{auto} & The band in \AA$^{-1}$ read at each atom;
\opt{auto} is 95\,\% of the coarsest training grid's Nyquist frequency. \\
\opt{occupancy\_weight} & \opt{1.0} & The per-atom term against the field
objective, and in the validation score. \\
\bottomrule
\end{tabular}
\end{center}

\opt{ext2paw} is not a link of the chain --- \opt{all} still means
\opt{ext2chg} then \opt{chg2tau} --- and the two switches are checked against
each other: \opt{task.type: ext2paw} with \opt{enable: false} raises, and so
does \opt{enable: true} beside \opt{ext2chg} or \opt{chg2tau}.

The operator is built as \file{experiments/paw\_occupancies} measured it should
be. \textbf{An FNO for the grid}, sized by the block; every other \opt{model}
setting applies to it unchanged. \textbf{A readout at each atom} of the latent
field and the predicted density, onto solid-harmonic Gaussians
$r^{L}Y_{LM}(\hat r)\,e^{-r^{2}/2s_n^{2}}$ --- the decoder half of a graph neural
operator --- over one band $|\GG| \le$ \opt{readout\_g\_max}, resampled to a
common spacing so every cell is integrated alike. \textbf{An equivariant head per
element}: one map per L shared across M, gated by invariants. With
\opt{model.equivariant} on, the whole operator rotates its predicted occupancies
with the crystal, to $7\times10^{-15}$ in double precision.

\begin{pwarn}[title={The band is not optional}]
A readout of the DFT density integrated on each grid as it came scored 3\,\% on
nanoparticles it had seen and 184\,\% on held-out ones, whose grid spacing
differs; over one band it scored 4.9\,\%. A grid too coarse to supply the band
therefore raises, at training and at inference.
\end{pwarn}

The records train with L = 0 centred and every (set, L, pair) block scaled to
unit RMS, fitted on the training split and inverted from the checkpoint, and
every L block weighs the same in the loss. The log gains a \opt{val occ}
column, and the best epoch is chosen on \opt{val rel L2 + occupancy\_weight
$\times$ val occ}. Each element's record layout comes from the POTCAR's
\opt{Non local Part}, so a dataset without POTCARs cannot train it; nor, yet,
can \opt{--kfold}, fine-tuning or an f-channel element, all refused before the
cache. A cache built before 16 September 2026 dropped the records; delete it and
let it rebuild.
...
process. The whole run's cost, stage by stage, closes the log
(Section~\ref{sec:resource-profile}).
...
\subsection{The resource profile}
\label{sec:resource-profile}

The last thing \opt{poraque-train} prints is what the run cost:
...
after a device synchronisation, so a GPU stage is charged for its compute rather
than for the kernels it queued. The memory columns answer the question an
allocation is sized from:

\begin{description}
  \item[peak RSS] is the process's high-water resident set, read from the
    operating system rather than sampled, so a spike between two samples cannot
    be missed. It is a running maximum and never falls; the stage where it
    jumps is the one that needed the memory. DataLoader workers are separate
    processes and are reported on a line of their own.
  \item[peak VRAM], on CUDA, is \opt{torch.cuda.max\_memory\_allocated}
    \emph{within} each stage --- the peak statistics are reset at every
    boundary, so the column does fall. The summary adds the most the caching
    allocator reserved, which is the number \opt{nvidia-smi} shows and the one
    an out-of-memory error is about.
  \item[MPS at end] is what the Metal driver held when the stage ended. MPS
    exposes no peak, so on a Mac this column is a floor, not a maximum.
\end{description}

\begin{pwarn}
The summary prints from a \opt{finally}, so a run that raises --- an
out-of-memory error, a bad file, a Ctrl-C --- still reports every stage it
finished and marks the one it died in \opt{(interrupted)}: the run whose cost
most needs explaining is the one that did not finish. A job killed by a signal
is not so lucky. Slurm ends a job at its time limit with \opt{SIGTERM}, which
Python does not turn into an exception, so that run leaves no summary; the
metrics JSON of a finished run is the record to size the next allocation from.
\end{pwarn}

\opt{--cache-only} prints the summary too, with the cache as its only stage.
The same record is in the metrics JSON under \opt{resources}; \opt{--kfold}
times the whole cross-validation of each task as one stage.
...
\begin{description}
  ...
  \item[The parity plot] is one row per \emph{occupied} bin: \opt{split},
    ...
    unrecoverable from the centres. \opt{count} is what the figure drew, so on
    log axes it leaves out every sampled voxel at which reference or prediction
    is not positive; the panel titles give the sample itself. A spin-polarised
    run writes the same pair again for $m$, as
    \file{<task>\_parity\_magnetisation}.
