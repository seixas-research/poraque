...
training. \\
\opt{valid\_fraction} & \opt{0.0} & Fraction of structures drawn at random for
validation. \\
...
These keys decide \emph{what question the run answers}, and are the settings
...
\begin{description}
  ...
  \item[\opt{valid\_fraction}] The same, but drawing the structures at random,
    shuffling with \opt{seed}. \opt{0} keeps everything for training.
...
\begin{pnote}
\opt{holdout} and \opt{valid\_fraction} both define a validation split and are
\textbf{mutually exclusive} --- setting both is an error rather than a silent
precedence rule, so the protocol cannot depend on which key the reader happened
to look at. \opt{valid\_fraction} always keeps at least one structure on each
side, so a fraction that would round to zero gives 1 rather than degrading into
a universal fit while the config claims otherwise.
\end{pnote}
