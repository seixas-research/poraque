...
\opt{training.physics\_informed\_setup} constrains the \textbf{neural
...
and \opt{training.physics\_informed} decides whether any of them is evaluated
--- \opt{auto} answers from the weights, \opt{true} refuses to be an empty
claim, \opt{false} also skips the per-batch decode the terms act on.
