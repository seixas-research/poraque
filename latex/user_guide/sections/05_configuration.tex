...
\opt{use\_vasp\_extcar} & \opt{false} & Read a reference \file{EXTCAR} instead
of computing one. \\
...
\subsection{\opt{use\_vasp\_extcar}}

\begin{pnote}
Off by default because standard VASP does not write \file{EXTCAR}. Poraqu\^e
reconstructs the external potential from the \file{POTCAR} tables to a relative
error of order $10^{-5}$, so the default costs essentially nothing in fidelity
and the pipeline runs on any standard VASP output. Setting this to \opt{true}
...
the tabulated pseudopotential, never in place of it. Both keys are ignored when
\opt{use\_vasp\_extcar} is set, since a reference file is taken as given.
...
\file{res32\_poraqueext\_blur0.15spec}, so changing them cannot silently reuse
a cache built with different settings.
\end{ptip}
