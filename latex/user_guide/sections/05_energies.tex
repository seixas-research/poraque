...
density is neither a PBE energy nor an LDA one. On the reference Pt supercells
...
\opt{ZVAL} --- 297 for the 27 Pt atoms above --- to a few parts in $10^{4}$. A
...
\begin{enumerate}
  \item \file{<dir>/Pt/POTCAR} --- what VASP ships;
  \item \file{<dir>/Pt\_pv/POTCAR} --- a variant, used only if it is the
    \emph{only} candidate, with a warning;
  \item \file{<dir>/POTCAR.Pt} or \file{<dir>/Pt.POTCAR} --- flat.
\end{enumerate}
...
$\tau$ and $V_{\rm loc}$ on a grid. Measured against VASP on platinum, using
...
\begin{tabular}{@{}lll@{}}
\toprule
\opt{method} & $E_{\rm iso}$ from & Pt cohesive energy \\
\midrule
...
density, so the PAW one-centre terms are absent entirely. For the 27-atom Pt
...
current seventeen-structure Pt dataset, within each composition group:
