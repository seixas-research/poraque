...
$E_{\rm xc}$ & Dirac exchange $+$ PW92 correlation (LDA) \\
...
\opt{potcar} & \file{POTCAR} for the species present --- \textbf{strongly
preferred}. \\
\opt{charges} & \opt{\{element: Z\_val\}}, used only without \opt{potcar}. \\
\opt{resolution} & Longest grid axis; defaults to the checkpoint's training
resolution. \\
\opt{functional} & Exchange-correlation approximation. \\
...
\begin{pwarn}
Without \opt{potcar} the external potential falls back to the \emph{Gaussian}
pseudo-ion model, which differs from the tabulated potential by a relative
$L^{2}$ of about $0.13$ --- against $2\times10^{-5}$ for the tabulated one. The
operators were trained on tabulated potentials, so the Gaussian model feeds
them an input outside their training distribution. The calculator warns once
and proceeds; treat the result as a smoke test, not a prediction.
\end{pwarn}
...
\paragraph{Energy differences are not usable yet either.} Measured on the seven
...
The error is roughly \textbf{three times the signal}. The cause is
cancellation, not a bug: the total is a sum of terms of order $10^{4}$ eV whose
physically relevant variation is a few eV, a relative $2.5\times10^{-4}$. A
field-level relative $L^{2}$ of $3\times10^{-2}$ cannot survive that. Reaching
$0.01$ eV/atom would need densities accurate to roughly $10^{-5}$ relative,
three orders of magnitude beyond the current models.
